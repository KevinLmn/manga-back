...
export class ImageService {
    ...
    while (retries < ImageService.MAX_RETRIES) {
      try {
        ...
        if (retries >= ImageService.MAX_RETRIES) {
          ...
          throw new ApiError(
            `Failed to download image ${index + 1}/${total}: ${error.message}`
          )
        }
  ...
  private async processImages(imageBuffers: Buffer[]): Promise<Buffer> {
    ...
    try {
      ...
      const images = await Promise.all(
        imageBuffers.map(async (buffer: Buffer, index: number) => {
          if (!buffer || buffer.length === 0) {
            throw new ApiError(`Image buffer ${index} is empty or invalid`, 500)
          }
          ...
          try {
            ...
            throw new ApiError(
              `Failed to process image ${index}: ${error.message}`,
              500
            )
          }
        })
      ...
      throw new ApiError(`Failed to process images: ${error.message}`, 500)
    }
  }
